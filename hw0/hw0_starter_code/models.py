...
class MultiTaskNet(nn.Module):
    ...
    def __init__(self, num_users, num_items, embedding_dim=32, layer_sizes=[96, 64],
        ...
        self.B, self.U_B = self.init_item_bias(num_users, num_items)
    ...
    def init_item_bias(self, num_users, num_items):
        ...
        # Add user bias items, see if it improves the model
        U_B = ZeroEmbedding(num_users, 1)
    ...
    def forward_with_embedding_sharing(self, user_ids, item_ids):
        ...
        # Regression head
        predictions = (
            torch.sum(self.U(user_ids) * self.Q(item_ids), dim=1)
            + self.B(item_ids).squeeze()
            + self.U_B(user_ids).squeeze()
        )  # (batch, 1) -> (batch,)

        # Matrix Factorization Head
        x = torch.cat([self.U(user_ids), self.Q(item_ids), self.U(user_ids) * self.Q(item_ids)], dim=1)
    ...
    def forward_without_embedding_sharing(self, user_ids, item_ids):
        ...
        # Regression head
        predictions = (
            torch.sum(self.U_reg(user_ids) * self.Q_reg(item_ids), dim=1)
            + self.B(item_ids).squeeze()
            + self.U_B(user_ids).squeeze()
        )

        # Matrix Factorization Head
        x = torch.cat([self.U_fact(user_ids), self.Q_fact(item_ids), self.U_reg(user_ids) * self.Q_reg(item_ids)], dim=1)
        for layer in self.mlp_layers:
            x = layer(x)
        score = x.squeeze()
