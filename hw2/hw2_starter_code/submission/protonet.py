    ...
    def _step(self, task_batch):
        ...
        for i, task in enumerate(task_batch):
            ...
            with torch.no_grad():
                ...
                support_features = self._network(images_support) # (num_way * num_support, latents)

                # Calculate prototypes from features
                # use labels_support to get label, then use labels to distill the number of classes
                prototypes = []
                for i in range(labels_support.max().item() + 1):
                    prototypes.append(support_features[labels_support == i].mean(dim=0))
                prototypes = torch.stack(prototypes) # (num_way, latents)

                # Calculate features for support
                distance_support = torch.cdist(support_features, prototypes, p=2) # (num_way * num_support, num_way)
                accuracy_support_batch.append(util.score(-distance_support, labels_support))

            # Generate query features
            query_features = self._network(images_query) # (num_way * num_query, latents)
            
            # Calculate probabilities
            distance_query = torch.cdist(query_features, prototypes, p=2) # (num_way * num_query, num_way)
            log_probs = F.log_softmax(-distance_query, dim=-1) # (num_way * num_query, num_way)
            accuracy_query_batch.append(util.score(-distance_query, labels_query))

            # Compute loss
            loss = F.nll_loss(log_probs, labels_query)
