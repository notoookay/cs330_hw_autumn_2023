...
class MANN(nn.Module):
    def __init__(self, num_classes, samples_per_class, hidden_dim, num_layers):
        super(MANN, self).__init__()
        self.num_classes = num_classes
        self.samples_per_class = samples_per_class

        self.layer1 = torch.nn.LSTM(num_classes + 784, hidden_dim, batch_first=True, num_layers=num_layers)
        self.layer2 = torch.nn.LSTM(hidden_dim, num_classes, batch_first=True, num_layers=num_layers)
    ...
    def loss_function(self, preds, labels):
        ...
        query_preds = preds[:, -1].reshape(-1, preds.size(-1))

        # Step 2: extract the true labels for the query set and reverse the one hot-encoding  
        query_labels = labels[:, -1].argmax(dim=-1).reshape(-1)
