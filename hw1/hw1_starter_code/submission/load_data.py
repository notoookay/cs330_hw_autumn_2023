...
class DataGenerator(IterableDataset):
    ...
    def _sample(self):
        ...
        image_batch = np.array([self.image_file_to_array(image, self.dim_input) for label, image in labels_and_images],
                               dtype=np.float32) # (K+1 * N, 784)
        label_batch = np.array([np.eye(self.num_classes)[label] for label, image in labels_and_images],
                               dtype=np.float32) # (K+1 * N, N)
        image_batch = image_batch.reshape(-1, self.num_samples_per_class, self.dim_input).transpose(1, 0, 2) # (K+1, N, 784)
        label_batch = label_batch.reshape(-1, self.num_samples_per_class, self.num_classes).transpose(1, 0, 2) # (K+1, N, N)
        
        # Step 4: Shuffle the order of examples from the query set
        # TODO: test the effect of shuffling the query set, comment the following to NOT shuffle
        idx = np.arange(self.num_classes)
        np.random.shuffle(idx)
        image_batch[-1] = image_batch[-1, idx]
        label_batch[-1] = label_batch[-1, idx]
